...
impl App {
    pub fn new(es_host: Host, action_buffer_size: usize, index: &str, page_size: usize, buffer_size: usize, del_timeout: u64, agg_timeout: u64) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    pub async fn run(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _del_handle = tokio::spawn(async move {
            if let Err(e) = delete_records_from_index(_es_host,_index.as_str(), buffer_size, del_timeout, delete_rx).await {
        ...
        let _agg_handle = tokio::spawn(async move {
            ...
            if let Err(e) = get_aggs_entries_from_index(_es_host, _index.as_str(), page_size, agg_timeout,_event_tx).await {
        ...
        loop {
            if let Some(event) = event_rx.recv().await {
                let _es_host = self.es_host.clone();
                if let Err(e) = self.process_events(_es_host, event, &event_tx, &delete_tx, &mut handles, _index.as_str()).await {
    ...
    async fn process_events(&mut self, es_host: Host, event: Message, event_tx: &mpsc::Sender<Message>, delete_tx: &mpsc::Sender<Value>, handles: &mut Vec<JoinHandle<()>>, index: &str) -> Result<(), Box<dyn std::error::Error>> {
        let _event_tx = event_tx.clone();
        match event {
